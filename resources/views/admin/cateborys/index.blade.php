    ...
    <div class="flex justify-between overflow-x-auto relative space-x-4">
        ...
        <div class="flex flex-col w-full lg:w-10/12 mt-28 h-full  p-4 min-h-screen">
            <div class="flex justify-between items-center">
                <p class="text-xl text-white dark:text-gray-600"> دسته بندی ها - {{ $cateborys->total() }}</p>
                <a href="{{ route('cateborys.create') }}"
                ...
                <table class="w-full text-right bg-coin1 dark:bg-gray-100 text-white dark:text-gray-600">
                    ...
                    <tbody>
                        @foreach ($cateborys as $key => $catebory)
                            <tr class=" border-b bg-coin1 dark:bg-gray-100 text-white dark:text-gray-600">
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div
                                        class="{{ $catebory->getRawOriginal('is_active') ? 'text-green-500' : 'text-red-500' }}">
                                        {{ $catebory->is_active }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 flex gap-4">
                                    <a class="flex bg-green-500 text-white px-4 py-2 rounded-md"
                                        href="{{ route('cateborys.edit', ['catebory' => $catebory->id]) }}">
                                        ویرایش
                                    </a>
                                </td>
