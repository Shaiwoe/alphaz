        ...
        <div class="flex flex-col sm:w-full md:w-9/12 lg:w-10/12 h-full m-0 overflow-hidden overflow-y-auto p-4">
            ...
            <div class="flex flex-col space-y-8 w-full">
                <div class="flex justify-between items-center">
                    <p class="text-xl text-white dark:text-zinc-900">ویرایش تگ {{ $tab->title }}</p>
                </div>
                {{-- form create article  --}}

                <form action="{{ route('tabs.update', ['tab' => $tab->id]) }}" method="POST">
                    @csrf
                    @method('put')
                    <div class="flex w-full gap-6  mb-6">
                        ...
                        <div class="flex flex-col w-full lg:w-3/12">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ $tab->slug }}"
                                class="block p-2.5 w-full rounded-3xl bg-coin1 dark:bg-slate-200 text-white dark:text-zinc-900 border-gray-600 placeholder-gray-400  focus:ring-yellow-400 focus:border-yellow-400"
                                placeholder="عنوان تگ را وارد کنید">
