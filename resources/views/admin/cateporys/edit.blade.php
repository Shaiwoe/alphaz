            ...
            <div class="flex flex-col w-full h-screen overflow-y-auto p-4">
                ...
                <div class="flex flex-col space-y-12 w-full px-4 mt-8">
                    <div class="flex justify-between items-center">
                        <p class="text-xl">ویرایش دسته بندی {{ $catepory->title }}</p>
                    </div>
                    {{-- form create article  --}}

                    <form action="{{ route('cateporys.update' , ['catepory' => $catepory->id ]) }}" method="POST">
                        @csrf
                        @method('put')
                        <div class="flex w-full gap-6  mb-6">
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <input type="text" name="title" id="title" value="{{ $catepory->title }}" class="block p-2.5 w-full rounded-lg bg-dark2 border-gray-600 placeholder-gray-400 text-white focus:ring-yellow-400 focus:border-yellow-400" placeholder="عنوان مقاله را وارد کنید" >
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>

                            <div class="flex flex-col w-full">
                                <label for="slug" class="block mb-2 text-sm font-medium text-gray-100 ">لینک</label>
                                <input type="slug" name="slug" id="slug" value="{{ $catepory->slug }}" class="block p-2.5 w-full rounded-lg bg-dark2 border-gray-600 placeholder-gray-400 text-white focus:ring-yellow-400 focus:border-yellow-400" placeholder="عنوان مقاله را وارد کنید" >
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <select id="parent_id" name="parent_id" class="flex  p-2.5 w-full rounded-lg bg-dark2 items-center justify-center text-center focus:ring-yellow-400 focus:border-yellow-400">
                                    ...
                                    @foreach ($cateporys as $catepory)
                                        <option value="{{ $catepory->id }}" {{ $catepory->parent_id == $catepory->id ? 'selected' : '' }}>
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <select id="is_active" id="is_active" name="is_active" class="flex  p-2.5 w-full rounded-lg bg-dark2 items-center justify-center text-center focus:ring-yellow-400 focus:border-yellow-400">
                                    <option value="1" {{ $catepory->getRawOriginal('is_active') ? 'selected' : '' }}>انتشار</option>
                                    <option value="0" {{ $catepory->getRawOriginal('is_active') ? '' : 'selected' }}>پیش نمایش</option>
                                </select>
                            </div>
