    ...
    <div class="flex justify-between overflow-x-auto relative space-x-4">
        ...
        <div class="flex flex-col w-full lg:w-10/12  h-full  p-4 mt-24 min-h-screen">
                <div class="flex justify-between items-center">
                    <p class="text-xl"> متاورس- {{ $metavers->total() }}</p>
                    <a href="{{ route('metavers.create') }}"
                        ...
                        <tbody>
                            @foreach ($metavers as $key => $metaver)
                                <tr
                                    ...
                                    <td class="px-6 py-4">
                                        <div
                                            class="{{ $metaver->getRawOriginal('is_active') ? 'text-green-500' : 'text-red-500' }}">
                                            {{ $metaver->is_active }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 flex gap-4">
                                        ...
                                        <form action="{{ route('metavers.destroy', ['metaver' => $metaver->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <a class="flex bg-green-500 text-white px-4 py-2 rounded-md"
                                            href="{{ route('metavers.edit', ['metaver' => $metaver->id]) }}">
                                            ویرایش
                                        </a>
