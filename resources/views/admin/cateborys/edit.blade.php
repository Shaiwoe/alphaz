                    ...
                    <form action="{{ route('cateborys.update' , ['catebory' => $catebory->id ]) }}" method="POST">
                        ...
                        <div class="flex w-full gap-6  mb-6">
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <input type="text" name="title" id="title" value="{{ $catebory->title }}" class="block p-2.5 w-full rounded-lg bg-dark2 border-gray-600 placeholder-gray-400 text-white focus:ring-yellow-400 focus:border-yellow-400" placeholder="عنوان مقاله را وارد کنید" >
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>

                            <div class="flex flex-col w-full">
                                <label for="slug" class="block mb-2 text-sm font-medium text-gray-100 ">لینک</label>
                                <input type="slug" name="slug" id="slug" value="{{ $catebory->slug }}" class="block p-2.5 w-full rounded-lg bg-dark2 border-gray-600 placeholder-gray-400 text-white focus:ring-yellow-400 focus:border-yellow-400" placeholder="عنوان مقاله را وارد کنید" >
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <select id="parent_id" name="parent_id" class="flex  p-2.5 w-full rounded-lg bg-dark2 items-center justify-center text-center focus:ring-yellow-400 focus:border-yellow-400">
                                    ...
                                    @foreach ($cateborys as $catebory)
                                        <option value="{{ $catebory->id }}" {{ $catebory->parent_id == $catebory->id ? 'selected' : '' }}>
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <select id="is_active" id="is_active" name="is_active" class="flex  p-2.5 w-full rounded-lg bg-dark2 items-center justify-center text-center focus:ring-yellow-400 focus:border-yellow-400">
                                    <option value="1" {{ $catebory->getRawOriginal('is_active') ? 'selected' : '' }}>انتشار</option>
                                    <option value="0" {{ $catebory->getRawOriginal('is_active') ? '' : 'selected' }}>پیش نمایش</option>
                                </select>
                            </div>
