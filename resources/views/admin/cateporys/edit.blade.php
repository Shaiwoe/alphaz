...
<head>
    ...
    <title>ویرایش دسته بندی پادکست</title>
</head>
    ...
    <div class="flex justify-between overflow-x-auto relative space-x-4">
        ...
        <div class="flex flex-col w-full lg:w-10/12 mt-28 h-full  p-4 min-h-screen">
            ...
            <div class="flex justify-between items-center">
                <p class="text-xl text-white">ویرایش دسته بندی {{ $catepory->title }}</p>
            </div>
            {{-- form create article  --}}
            ...
            <form action="{{ route('cateporys.update', ['catepory' => $catepory->id]) }}" method="POST">
                @csrf
                @method('put')
                <div class="flex flex-col lg:flex-row w-full gap-6 mt-12 mb-6">

                    <div class="flex flex-col w-full">
                        <label for="title" class="block mb-2 text-sm font-medium text-gray-100 ">عنوان دسته
                            بندی</label>
                        <input type="text" name="title" id="title" value="{{ $catepory->title }}"
                    ...
                    <div class="flex flex-col w-full">
                        ...
                        <input type="slug" name="slug" id="slug" value="{{ $catepory->slug }}"
                            class="block p-2.5 w-full rounded-lg bg-coin1 dark:bg-gray-100 text-white dark:text-gray-600 border-gray-600 placeholder-gray-400  focus:ring-yellow-400 focus:border-yellow-400"
                            placeholder="عنوان مقاله را وارد کنید">
                        ...
                        <select id="parent_id" name="parent_id"
                            ...
                            @foreach ($cateporys as $catepory)
                                <option value="{{ $catepory->id }}"
                                    {{ $catepory->parent_id == $catepory->id ? 'selected' : '' }}>
                    ...
                    <div class="flex flex-col w-full">
                        ...
                        <select id="is_active" id="is_active" name="is_active"
                            class="flex  p-2.5 w-full rounded-lg bg-coin1 dark:bg-gray-100 text-gray-400 items-center justify-center text-center focus:ring-yellow-400 focus:border-yellow-400">
                            <option value="1" {{ $catepory->getRawOriginal('is_active') ? 'selected' : '' }}>انتشار
                            </option>
                            <option value="0" {{ $catepory->getRawOriginal('is_active') ? '' : 'selected' }}>پیش
                                نمایش</option>
                        </select>
                    </div>
