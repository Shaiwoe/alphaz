...
<body class="bg-white3 dark:bg-dark3">
    ...
    <div class="container mx-auto text-white p-4 sm:p-8 mt-12">
        ...
        {{-- slide top  --}}
        @foreach ($videosBanners as $videosBanner)
            <div class="flex flex-col-reverse lg:flex-row w-full items-center lg:gap-12">
                <div class="flex flex-col z-40 space-y-4 sm:space-y-8 w-full">
                    <p class="flex items-center gap-2 mt-4 text-xs lg:text-sm text-gray-800 dark:text-white">
                        ...
                        {{ verta($videosBanner->created_at) }}
                    </p>
                    <a href="{{ route('home.videos.show', ['video' => $videosBanner->slug]) }}" class="text-base md:text-3xl text-gray-800 dark:text-white">{{ $videosBanner->title }}</a>
                    <p class="text-sm  text-gray-800 dark:text-white">{{ $videosBanner->description }}</p>
                    <div class="flex justify-start">
                        <a href="{{ route('home.videos.show', ['video' => $videosBanner->slug]) }}"
                ...
                <a href="{{ route('home.videos.show', ['video' => $videosBanner->slug]) }}" class="flex w-full z-40">
                    <image class="rounded-lg" src="{{ asset(env('VIDEO_IMAGES_UPLOAD_PATH') . $videosBanner->image) }}" alt="">
                </a>
            </div>
        @endforeach
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 w-full mt-20 gap-8">
            ...
            @foreach ($videos as $video)
                <div class="flex w-full h-full flex-col z-40 space-y-2 shadow-lg shadow-yellow-500 rounded-lg bg-white dark:bg-dark3 text-gray-700 dark:text-white">
                    <a href="{{ route('home.videos.show', ['video' => $video->slug]) }}" class="w-full p-3">
                        <image class="w-full h-44 max-h-44"
                            src="{{ asset(env('VIDEO_IMAGES_UPLOAD_PATH') . $video->image) }}"
                            alt="{{ $video->title }}">
                    </a>
                    <div class="w-full space-y-6 px-3">
                        <a href="{{ route('home.videos.show', ['video' => $video->slug]) }}" class="text-lg font-bold">{{ $video->title }}</a>
                    ...
                    <div class="flex flex-col w-full gap-4 p-3">
                        ...
                        <div class="flex w-full bg-yellow-400 justify-center rounded-md p-1">
                            <a href="{{ route('home.videos.show', ['video' => $video->slug]) }}">ادامه مطلب</a>
                        </div>
                    </div>
                </div>
            @endforeach
