...
<body class="bg-white3 dark:bg-dark3">
    ...
    <div class="container mx-auto text-white p-4 sm:p-8 mt-12">
        ...
        {{-- slide top  --}}
        @foreach ($padcastsBanners as $padcastsBanner)
            <div class="flex flex-col-reverse lg:flex-row w-full items-center lg:gap-12">
                <div class="flex flex-col z-40 space-y-4 sm:space-y-8 w-full">
                    <p class="flex items-center gap-2 mt-4 text-xs lg:text-sm text-gray-800 dark:text-white">
                        ...
                        {{ verta($padcastsBanner->created_at) }}
                    </p>
                    <a href="{{ route('home.padcasts.show', ['padcast' => $padcastsBanner->slug]) }}" class="text-base md:text-3xl text-gray-800 dark:text-white">{{ $padcastsBanner->title }}</a>
                    <p class="text-sm  text-gray-800 dark:text-white">{{ $padcastsBanner->description }}</p>
                    <div class="flex justify-start">
                        <a href="{{ route('home.padcasts.show', ['padcast' => $padcastsBanner->slug]) }}"
                ...
                <a href="{{ route('home.padcasts.show', ['padcast' => $padcastsBanner->slug]) }}" class="flex w-full z-40 justify-end">
                    <image class="rounded-lg" src="{{ asset(env('PADCAST_IMAGES_UPLOAD_PATH') . $padcastsBanner->image) }}" alt="">
                </a>
            </div>
        @endforeach
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 w-full mt-20 gap-8">
            ...
            @foreach ($padcasts as $padcast)
                <div class="flex w-full h-full flex-col z-40 space-y-2 shadow-lg shadow-yellow-500 rounded-lg bg-white dark:bg-dark3 text-gray-700 dark:text-white">
                    <a href="{{ route('home.padcasts.show', ['padcast' => $padcast->slug]) }}" class="w-full p-3">
                        <image class="w-full h-44 max-h-44"
                            src="{{ asset(env('PADCAST_IMAGES_UPLOAD_PATH') . $padcast->image) }}"
                            alt="{{ $padcast->title }}">
                    </a>
                    <div class="w-full space-y-6 px-3">
                        <a href="{{ route('home.padcasts.show', ['padcast' => $padcast->slug]) }}" class="text-lg font-bold">{{ $padcast->title }}</a>
                        <p class="text-sm text-gray-400">{{ $padcast->description }}</p>
                    </div>
                    <div class="flex flex-col w-full gap-4 p-3">
                        ...
                        <div class="flex w-full bg-yellow-400 justify-center rounded-md p-1">
                            <a href="{{ route('home.padcasts.show', ['padcast' => $padcast->slug]) }}">ادامه مطلب</a>
                        </div>
                    </div>
                </div>
            @endforeach
